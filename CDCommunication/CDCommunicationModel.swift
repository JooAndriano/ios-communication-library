...
@objc public class CDCommunicationActivity: NSObject {
...
@objc public class CDCommunicationComments: NSObject {
...
@objc public class CDCommunicationEditorDetailsCreators: NSObject {
...
@objc public class CDCommunicationEditorDetailsEditors: NSObject {
...
@objc public class CDCommunicationEditorTemplates: NSObject {
...
@objc public class CDCommunicationExternalSite: NSObject {
...
@objc public class CDCommunicationFile: NSObject {
...
@objc public class CDCommunicationNotifications: NSObject {
...
@objc public class CDCommunicationRichdocumentsTemplate: NSObject {
...
@objc public class CDCommunicationShare: NSObject {
...
@objc public class CDCommunicationSharee: NSObject {
...
@objc public class CDCommunicationTrash: NSObject {
...
@objc public class CDCommunicationUserProfile: NSObject {
...
@objc public class CDCommunicationUserStatus: NSObject {
...
class CDDataFileXML: NSObject {

    let requestBodyComments =
    """
    <?xml version=\"1.0\" encoding=\"UTF-8\"?>
    <d:propfind xmlns:d=\"DAV:\" xmlns:oc=\"http://owncloud.org/ns\" xmlns:nc=\"http://calldrive.org/ns\">
        <d:prop>
            <oc:id />
            <oc:verb />
            <oc:actorType />
    ...
    """
    <?xml version=\"1.0\" encoding=\"UTF-8\"?>
    <d:propertyupdate xmlns:d=\"DAV:\" xmlns:oc=\"http://owncloud.org/ns\" xmlns:nc=\"http://calldrive.org/ns\">
        <d:set>
            <d:prop>
    ...
    <d:propfind xmlns:d=\"DAV:\" xmlns:oc=\"http://owncloud.org/ns\" xmlns:nc=\"http://calldrive.org/ns\">
        <d:prop>
            ...
            <creation_time xmlns=\"http://calldrive.org/ns\"/>
            <upload_time xmlns=\"http://calldrive.org/ns\"/>
            <is-encrypted xmlns=\"http://calldrive.org/ns\"/>
            <has-preview xmlns=\"http://calldrive.org/ns\"/>
            <mount-type xmlns=\"http://calldrive.org/ns\"/>
            <rich-workspace xmlns=\"http://calldrive.org/ns\"/>
            <note xmlns=\"http://calldrive.org/ns\"/>
    ...
    """
    <?xml version=\"1.0\"?>
    <oc:filter-files xmlns:d=\"DAV:\" xmlns:oc=\"http://owncloud.org/ns\" xmlns:nc=\"http://calldrive.org/ns\">
        <d:prop>
            <d:getlastmodified />
            <d:getetag />
            <d:getcontenttype />
    ...
    """
    <?xml version=\"1.0\"?>
    <d:searchrequest xmlns:d=\"DAV:\" xmlns:oc=\"http://owncloud.org/ns\" xmlns:nc=\"http://calldrive.org/ns\">
    <d:basicsearch>
        <d:select>
            <d:prop>
                <d:displayname/>
                ...
                <comments-unread xmlns=\"http://owncloud.org/ns\"/>
                <has-preview xmlns=\"http://calldrive.org/ns\"/>
                <trashbin-filename xmlns=\"http://calldrive.org/ns\"/>
                <trashbin-original-location xmlns=\"http://calldrive.org/ns\"/>
                <trashbin-deletion-time xmlns=\"http://calldrive.org/ns\"/>
            </d:prop>
        </d:select>
    <d:from>
        <d:scope>
    ...
    func convertDataFile(data: Data, showHiddenFiles: Bool) -> [CDCommunicationFile] {
        
        var files: [CDCommunicationFile] = []
        var dicMOV: [String:Int] = [:]
        var dicImage: [String:Int] = [:]
        let webDavRoot = "/" + CDCommunicationCommon.shared.webDav + "/"
        let davRootFiles = "/" + CDCommunicationCommon.shared.dav + "/files/"
        guard let baseUrl = CDCommunicationCommon.shared.getHostName(urlString: CDCommunicationCommon.shared.urlBase) else {
            return files
        }
        ...
        for element in elements {
            ...
            if let href = element["d:href"].text {
                ...
                } else if file.path.contains(davRootFiles + CDCommunicationCommon.shared.user) {
                    let postUrl = file.path.replacingOccurrences(of: davRootFiles + CDCommunicationCommon.shared.user, with: webDavRoot.dropLast())
                    file.serverUrl = baseUrl + postUrl.dropLast()
                } else if file.path.contains(davRootFiles + CDCommunicationCommon.shared.userId) {
                    let postUrl = file.path.replacingOccurrences(of: davRootFiles + CDCommunicationCommon.shared.userId, with: webDavRoot.dropLast())
                    file.serverUrl = baseUrl + postUrl.dropLast()
                }
            ...
            if let getlastmodified = propstat["d:prop", "d:getlastmodified"].text {
                if let date = CDCommunicationCommon.shared.convertDate(getlastmodified, format: "EEE, dd MMM y HH:mm:ss zzz") {
                    file.date = date
                }
            }
            ...
            let results = CDCommunicationCommon.shared.getInternalType(fileName: file.fileName, mimeType: file.contentType, directory: file.directory)
            ...
            file.urlBase = CDCommunicationCommon.shared.urlBase
            ...
            } else if file.typeFile == CDCommunicationCommon.typeFile.image.rawValue {
    ...
    func convertDataTrash(data: Data, showHiddenFiles: Bool) -> [CDCommunicationTrash] {
        
        var files: [CDCommunicationTrash] = []
        var first: Bool = true
        guard let baseUrl = CDCommunicationCommon.shared.getHostName(urlString: CDCommunicationCommon.shared.urlBase) else {
            return files
        }
    ...
    func convertDataComments(data: Data) -> [CDCommunicationComments] {
        
        var items: [CDCommunicationComments] = []
    
        let xml = XML.parse(data)
        let elements = xml["d:multistatus", "d:response"]
        for element in elements {
            let item = CDCommunicationComments()
            ...
            if let creationDateTime = element["d:propstat", "d:prop", "d:creationDateTime"].text {
                if let date = CDCommunicationCommon.shared.convertDate(creationDateTime, format: "EEE, dd MMM y HH:mm:ss zzz") {
    ...
    func convertDataShare(data: Data) -> (shares: [CDCommunicationShare], statusCode: Int, message: String) {
        
        var items: [CDCommunicationShare] = []
        ...
        for element in elements {
            ...
            if let value = element["expiration"].text {
                if let date = CDCommunicationCommon.shared.convertDate(value, format: "YYYY-MM-dd HH:mm:ss") {
