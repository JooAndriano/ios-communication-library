//
//  CDCommunication+Richdocuments.swift
//  CDCommunication
//
...
extension CDCommunication {
    ...
    @objc public func getTemplatesRichdocuments(typeTemplate: String, customUserAgent: String? = nil, addCustomHeaders: [String: String]? = nil, completionHandler: @escaping (_ account: String, _ templates: [CDCommunicationRichdocumentsTemplate]?, _ errorCode: Int, _ errorDescription: String) -> Void) {
        ...
        sessionManager.request(url, method: method, parameters: nil, encoding: URLEncoding.default, headers: headers, interceptor: nil).validate(statusCode: 200..<300).responseJSON { (response) in
            ...
            case .success(let json):
                ...
                let statusCode = json["ocs"]["meta"]["statuscode"].int ?? CDCommunicationError().getInternalError()
                if statusCode == 200 {
                    var templates: [CDCommunicationRichdocumentsTemplate] = []
                    for templateJSON in data {
                        let template = CDCommunicationRichdocumentsTemplate()
